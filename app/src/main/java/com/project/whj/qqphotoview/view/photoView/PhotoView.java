...
public class PhotoView extends android.support.v7.widget.AppCompatImageView implements IPhotoView {
	...
	public void setOnLockListener(OnLockListener onLockListner) {
	...
	public PhotoView(Context context, AttributeSet attr, int defStyle) {
		...
		mAttacher.setOnMatrixChangeListener(new PhotoViewAttacher.OnMatrixChangedListener() {
			...
			public void onMatrixChanged(RectF rect) {
				if (mOnLockListner != null) {
					if (rect.left <= 0 || getWidth()>rect.width()) {
						mOnLockListner.onLock(false);
					} else {
						mOnLockListner.onLock(true);
					}
				}
			}
		});
	...
	public void setAbleZoomMinScale(float ableZommMinScale){
		mAttacher.setAbleZoomMinScale(ableZommMinScale);
    }
	
	@Override
	public float getAbleZoomMinScale() {
		return mAttacher.getAbleZoomMinScale();
	}
